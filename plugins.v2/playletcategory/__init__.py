...
class PlayletCategory(_PluginBase):
    ...
    def __get_duration(video_path: str) -> float:
        """
        获取视频文件时长（分钟）,失败时最多重试10次,每次失败增加5秒等待时间
        """
        retry_count = 0
        max_retries = 10
            ...
            try:
                ...
                if error:
                    ...
                    if retry_count < max_retries:
                        ...
                        logger.error(
                            f"第 {retry_count} 次获取视频时长失败: {error.decode('utf-8')}, {delay} 秒后重试...")
                        time.sleep(delay)
                        continue
                    else:
                        logger.error(f"获取视频时长已重试 {max_retries} 次仍失败: {error.decode('utf-8')}")
                        return 0
                ...
                if retry_count < max_retries:
                    # 计算本次等待时间
                    delay = base_delay * retry_count
                    logger.error(f"第 {retry_count} 次获取视频时长出现异常: {str(e)}, {delay} 秒后重试...")
                    time.sleep(delay)
                    continue
                else:
                    logger.error(f"获取视频时长已重试 {max_retries} 次仍出现异常: {str(e)}")
                    return 0
